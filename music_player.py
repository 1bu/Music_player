...
def get_song_path(song_name):
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(BASE_DIR, 'Music', song_name)
...
def toggle_play(btn_action, state, playlist,current_song):
    selected_file = playlist.get(tk.ACTIVE)

    #Si la cancion seleccionada es diferente
    if state["current_file"] != selected_file:
        play_song(state, selected_file, current_song,btn_action)
